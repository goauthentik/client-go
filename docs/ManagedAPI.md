...
> BlueprintImportResult ManagedBlueprintsImportCreate(ctx).File(file).Path(path).Context(context).Execute()
...
func main() {
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ManagedAPI.ManagedBlueprintsImportCreate(context.Background()).File(file).Path(path).Context(context).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ManagedAPI.ManagedBlueprintsImportCreate``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `ManagedBlueprintsImportCreate`: BlueprintImportResult
	fmt.Fprintf(os.Stdout, "Response from `ManagedAPI.ManagedBlueprintsImportCreate`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiManagedBlueprintsImportCreateRequest struct via the builder pattern
