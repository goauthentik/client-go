...
> SsfStreamsDestroy(ctx, uuid).Execute()
...
func main() {
	...
	r, err := apiClient.SsfAPI.SsfStreamsDestroy(context.Background(), uuid).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `SsfAPI.SsfStreamsDestroy``: %v\n", err)
...
Other parameters are passed through a pointer to a apiSsfStreamsDestroyRequest struct via the builder pattern
...
func main() {
	deliveryMethod := openapiclient.DeliveryMethodEnum("https://schemas.openid.net/secevent/risc/delivery-method/push") // DeliveryMethodEnum |  (optional)
